...
contract PluginRepoFactory {
    ...
    function createPluginRepo(address _initialOwner) external returns (PluginRepo) {
        return _createPluginRepo(_initialOwner);
    }

    /// @notice Creates and registers a `PluginRepo` and publishes an initial version `1.1`.
    ...
    function _createPluginRepo(address _initialOwner) internal returns (PluginRepo pluginRepo) {
        pluginRepo = PluginRepo(
            createERC1967Proxy(pluginRepoBase, abi.encodeWithSelector(PluginRepo.initialize.selector, _initialOwner))
        );
